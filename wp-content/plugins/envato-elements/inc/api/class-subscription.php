...
class Subscription extends API {
	...
	public function verify_extensions_token( $request ) {
		...
		$token_response = \Envato_Elements\Backend\Subscription::get_instance()->verify_token_and_cache_user_info($extensions_token);

		if($token_response['valid']){
			// we received a valid token from Extensions API
			if($token_response['status'] === \Envato_Elements\Backend\Subscription::get_instance()::SUBSCRIPTION_PAID) {
